...
router.get("/", async (req, res) => {
  ...
  const [[users], [favoriteCounts], [couponCounts], [orderCounts]] = await Promise.all([
    db.query<DbRow[]>("SELECT id, phone, nickname, avatar_url, platform, created_at FROM users WHERE id = ?", [userId]),
...
router.patch("/", async (req, res) => {
  ...
  const [rows] = await db.query<DbRow[]>("SELECT id, phone, nickname, avatar_url, platform, created_at FROM users WHERE id = ?", [requireUserId(req)]);
